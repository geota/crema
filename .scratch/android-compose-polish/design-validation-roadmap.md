...
5. ⏳ Profile editor: ✅ top-bar Discard/Duplicate/Save-profile + divider, serif "Pressure profile" curve header + sub + Reset, pin copy, per-segment cards (surfaceContainerHigh) w/ number pills. ⏳ DEFERRED: accordion collapse/expand (pill+summary+caret), SegFieldCell 3-up grid, TargetTile, flatten the section EditorCards, serif title/Bean/Notes fields, dashed flow ghost in editor curve.
6. ⏳ Settings: ✅ contained pass — "Preferences" eyebrow, large-radius group cards, SetRow molecule (title + subtitle + trailing control) for Theme + the 3 behaviour toggles (now with helper text). ⏳ DEFERRED: two-pane 248dp section-nav + 880 content, the 8 full sections (Machine hero/diagnostics, Brew defaults, Water&maint, Sharing, Calibration, Advanced, About), SetSelect/SetStepper/StatusDot molecules, divider-rows.
