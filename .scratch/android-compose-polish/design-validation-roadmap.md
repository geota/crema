...
5. ⏳ Profile editor: ✅ top-bar Discard/Duplicate/Save-profile + divider, serif "Pressure profile" curve header + sub + Reset, pin copy, per-segment cards (surfaceContainerHigh) w/ number pills. ✅ accordion collapse/expand (number pill + collapsed summary + rotating caret, one open at a time). ✅ Volume cell (SegmentEdit.volume → volumeLimitMl, full thin-model→patch→UI chain). ⏳ DEFERRED: Exit/Max cells (complex structured ExitCondition/limiter — need full wire modeling), 3-up SegFieldCell compaction, TargetTile, flatten section EditorCards, serif title/Bean/Notes fields, dashed flow ghost.
...
10. ✅ brand fonts via GoogleFont provider (res/values/font_certs.xml from android/compose-samples; Type.kt Newsreader/Hanken/JetBrains via GoogleFont, system fallback w/o GMS). ✅ History: StoredShot rating/notes + vm.updateShot + detail star-rating + notes field. ✅ Settings Machine diagnostics (scale battery/firmware). ✅ editor Volume + Exit/Max cells (full ExitCondition{metric,compare,threshold} + Limiter{value,range}, modeled off the web/Rust wire shape — zero core change). ✅ QuickControls Phase A: transient BrewParams (dose/yield/brew-temp) baked into the start-shot upload via overrideBrewParamsJson (the web shell's re-upload-with-overrides model; never mutates the profile, cleared on switch/Reset) + favorites strip (pinned profiles / favourite beans → setActive) + Reset + toggles. ✅ Save-preset (clone active profile → new custom carrying the dial values; pure shell via quickPresetJson + the duplicate/custom-store path). ⏳ OPTIONAL (NOT core-blocked): live mid-shot SAW dial + adjustable steam/water/flush param cards — yield/dose/temp are already covered by the upload-bake, so the ONLY thing that'd touch the uniffi bridge is exposing existing-core MMR setters IF adjustable steam/water is wanted. Nothing is required; the espresso-first flow is complete.
