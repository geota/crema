...
/// peak-weight yield fallback the web applied). See
/// [`de1_domain::history_stats`].
#[uniffi::export]
pub fn history_stats(shots_json: String) -> Result<String, CremaError> {
...
#[uniffi::export]
pub fn resolve_mode_targets(inputs_json: String) -> Result<String, CremaError> {
    let inputs: de1_domain::ModeTargetInputs =
        serde_json::from_str(&inputs_json).map_err(crema_err)?;
    serde_json::to_string(&de1_domain::resolve_mode_targets(&inputs)).map_err(crema_err)
}
...
impl CremaBridge {
    ...
    pub fn brew_session_arm(&self, recipe_json: String, start_on_pour: bool) -> String {
        self.emit(self.core().brew_session_arm(&recipe_json, start_on_pour))
    }

    /// Start an armed session's clock (the Start tap).
    pub fn brew_session_begin(&self, now_ms: u64) -> String {
        self.emit(self.core().brew_session_begin(now_ms))
    }

    /// Freeze a running session's clock.
    pub fn brew_session_pause(&self, now_ms: u64) -> String {
        self.emit(self.core().brew_session_pause(now_ms))
    }

    /// Resume a paused session.
    pub fn brew_session_resume(&self, now_ms: u64) -> String {
        self.emit(self.core().brew_session_resume(now_ms))
    }

    /// Advance to the next recipe step.
    pub fn brew_session_skip(&self, now_ms: u64) -> String {
        self.emit(self.core().brew_session_skip(now_ms))
    }

    /// End the session now; the output carries `BrewSessionCompleted`.
    pub fn brew_session_finish(&self, now_ms: u64) -> String {
        self.emit(self.core().brew_session_finish(now_ms))
    }

    /// Drop the session without a summary.
    pub fn brew_session_cancel(&self) -> String {
    ...
    /// or `None` if nothing matched. A distinctive discovered service wins over
    /// the name (Acaia generation, rebrand, mixed-case); pass an empty list for
    ...
    pub fn set_volume_stop_with_scale(&self, enabled: bool) {
    ...
    /// Session-independent: recompute-on-view, nothing is persisted.
    pub fn analyze_shot_quality(&self, input_json: String) -> Result<String, CremaError> {
        let input: de1_domain::shot_quality::ShotQualityInput =
            serde_json::from_str(&input_json).map_err(crema_err)?;
        serde_json::to_string(&de1_domain::shot_quality::analyze_shot(&input)).map_err(crema_err)
    }

    /// Analyze a persisted shot end-to-end: parse the core-shape
    /// `StoredShot` JSON, build the analysis input (real per-sample frame
    /// markers — review #39, replacing both shell mappers), and run the
    /// full detector cascade. Returns the report JSON, or the string
    /// `"null"` when the shot has too few samples to analyze (the shell
    /// hides the quality card).
    pub fn analyze_stored_shot_quality(&self, shot_json: String) -> Result<String, CremaError> {
        let shot: de1_domain::StoredShot = serde_json::from_str(&shot_json).map_err(crema_err)?;
        match de1_domain::shot_quality::analyze_stored_shot(&shot) {
            Some(report) => serde_json::to_string(&report).map_err(crema_err),
